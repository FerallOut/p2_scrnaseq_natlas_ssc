...
## TEST
test_ds="gse292979"
#list.files("../../data/internal/cellranger_count/")

in_cellranger_counts_multiple_obj <- "../../data/internal/cellranger_count/"
in_metadata <- "../../annotations/2_export/2025.10.26_public_scrnaseq_data.tsv"
in_metaDenton <- "../../annotations/1_raw/2025_10_31_SraRunTable.csv"
```
...
samples <- dir(path = in_cellranger_counts_multiple_obj, 
  ...
  grep(test_ds, ., value = TRUE) %T>%
  {cat(paste0("nr of samples: ", length(.), "\n")) } %T>% 
  {head(.) %>% print(.)} 
...
lapply(colnames(meta) %>% list(), function(x) meta %>%
         group_by(patient_id) %>%
         filter(n() > 1) %>%
         summarize(n = n()) ) 
...
## TEST
meta <- meta %>% 
  filter(dataset == test_ds %>% toupper()) %T>%
  {dim(.) %>% print(.)} %T>% 
  {tail(.) %>% print(.)}
```
...
seu_merge_unfilt <- merge(x=seu_obj_list[[1]], 
                   y=c(seu_obj_list[2:length(seu_obj_list)]), 
